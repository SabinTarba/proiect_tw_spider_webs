                ...
                <ul className="navbar-nav ml-auto">
                    <Nav className="me-auto navbar navbar-expand-lg">
                        ...
                        <li className="nav-item">
                            {loggedUser?.type === STUDENT_ACCOUNT && new Date() >= new Date(professorDueDate) && new Date() <= new Date().setDate(new Date(professorDueDate).getDate()) &&
                                <Button href={"/dashboard/grade-projects"} className="text-white fw-bold" variant="info">Grade projects ({`${Math.round((new Date().setDate(new Date(professorDueDate).getDate() + 3) - new Date()) / (1000 * 60 * 60 * 24))} day(s) left`})</Button>
                            }
                        </li>
                        <li className="nav-item">
                    ...
                    <li className="nav-item">
                        <Navbar.Collapse className="navbar-text ml-auto">
                            <Navbar.Brand className='navbar navbar-right'>
                                <span className="fw-bold text-white">Signed in as: <span className="text-warning fw-bold">{loggedUser?.lastName + " " + loggedUser?.firstName + " " + alias}</span></span>
